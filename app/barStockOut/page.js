...
const BarStockOut = () => {
  ...
  const fetchProductNames = async () => {
      try {
          const response = await axios.get("http://172.188.99.139:5000/api/liquorBrand/barSubmenu/list");
  ...
  const handleInputChange = async (e) => {
      ...
      try {
          if (name === "waiterName") {
              setWaiterName(value);
              const response = await axios.get(`http://172.188.99.139:5000/api/waiter/waiter/mobile?name=${value}`);
              setMobileNumber(response.data.mobileNumber);
          } else if (name === "productName") {
              setProductName(value);
              // Fetch and update available quantity based on the selected product name
              const response = await axios.get(`http://172.188.99.139:5000/api/barStockOut/purchase/childMenuStockQty?name=${value}`);
              setAvailableQuantity(response.data.stockQty || 0);
              setUnit(response.data.unit || "");
              // Fetch parentMenuId based on the selected product name
              const parentMenuResponse = await axios.get(`http://172.188.99.139:5000/api/barStockOut/getParentMenuId?productName=${value}`);
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          const responseAddItems = await axios.post("http://172.188.99.139:5000/api/barStockOut/stockOut/addItems", {
              waiterName,
              productName,
          ...
          const response = await axios.post("http://172.188.99.139:5000/api/barStockOut/barStockOut", {
