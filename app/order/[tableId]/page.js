    ...
    useEffect(() => {
        const fetchACPercentage = async () => {
            try {
                ...
                const tableResponse = await axios.get(`https://172.188.99.139:5001/api/table/tables/${tableId}`);
                const table = tableResponse.data;

                // Fetch the section information using the section ID
                const sectionResponse = await axios.get(`https://172.188.99.139:5001/api/section/${table.section._id}`);
