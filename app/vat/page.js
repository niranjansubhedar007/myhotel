...
const VATForm = ({ onSubmit }) => {
    ...
    useEffect(() => {
        const fetchVATPercentage = async () => {
            try {
                if (hotel._id) {
                    const vatResponse = await axios.get(`http://ec2-16-171-154-162.eu-north-1.compute.amazonaws.com:5000/api/hotel/get/${hotel._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (hotel._id && vatPercentage !== '') {
            try {
                await axios.patch(`http://ec2-16-171-154-162.eu-north-1.compute.amazonaws.com:5000/api/hotel/vat/${hotel._id}`, {
