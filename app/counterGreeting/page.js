...
const CounterGreetings = () => {
  ...
  const getGreetings = async () => {
    try {
      const response = await axios.get('https://172.188.99.139:5001/api/counterGreet/greet');
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const response = await axios.patch(
        `https://172.188.99.139:5001/api/counterGreet/greet/${editedGreet.id}`,
  ...
  //   try {
  //     await axios.delete(`https://172.188.99.139:5001/api/greet/greet/${itemIdToDelete}`);
  ...
  const handleDeleteConfirmed = async () => {
    try {
      await axios.delete(`https://172.188.99.139:5001/api/counterGreet/greet/${itemIdToDelete}`);
      setGreetings((prevGreetings) => prevGreetings.filter((greet) => greet._id !== itemIdToDelete));
      setIsDeleteConfirmationModalOpen(false);
  ...
  const createGreet = async () => {
    ...
    try {
      if (!dataAdded) {
        const response = await axios.post('https://172.188.99.139:5001/api/counterGreet/greet', {
          greet: trimmedGreet,
  ...
  const updateGreet = async (greetId) => {
    ...
    try {
      const response = await axios.patch(`https://172.188.99.139:5001/api/counterGreet/greet/${greetId}`, updateGreetData);
