...
const CounterGstForm = () => {
    ...
    useEffect(() => {
        const fetchHotelAndGSTPercentage = async () => {
            try {
                const hotelsResponse = await axios.get('http://ec2-16-171-154-162.eu-north-1.compute.amazonaws.com:5000/api/counterHotel/get-all');
    ...
    useEffect(() => {
        const fetchGSTPercentage = async () => {
            try {
                if (hotel._id) {
                    const gstResponse = await axios.get(`http://ec2-16-171-154-162.eu-north-1.compute.amazonaws.com:5000/api/counterHotel/get/${hotel._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (hotel._id && gstPercentage !== '') {
            try {
                await axios.patch(`http://ec2-16-171-154-162.eu-north-1.compute.amazonaws.com:5000/api/counterHotel/gst/${hotel._id}`, {
