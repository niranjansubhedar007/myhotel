...
const CounterGstForm = () => {
    ...
    useEffect(() => {
        const fetchHotelAndGSTPercentage = async () => {
            try {
                const hotelsResponse = await axios.get('https://172.188.99.139:5001/api/counterHotel/get-all');
    ...
    useEffect(() => {
        const fetchGSTPercentage = async () => {
            try {
                if (hotel._id) {
                    const gstResponse = await axios.get(`https://172.188.99.139:5001/api/counterHotel/get/${hotel._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (hotel._id && gstPercentage !== '') {
            try {
                await axios.patch(`https://172.188.99.139:5001/api/counterHotel/gst/${hotel._id}`, {
