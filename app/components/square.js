    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get('https://172.188.99.139:5001/api/hotel/counts');
                setCounts(response.data);

                // Fetch data for temporary orders count
                const temporaryOrdersResponse = await axios.get('https://172.188.99.139:5001/api/order/temporary-orders-count');
                ...
                const totalForCurrentDateResponse = await axios.get('https://172.188.99.139:5001/api/order/total-amount-for-current-date');
                setCounts(prevCounts => ({
                    ...prevCounts,
                ...
                const totalForPreviousMonthResponse = await axios.get('https://172.188.99.139:5001/api/order/total-amount-for-previous-month');
                setCounts(prevCounts => ({
                    ...prevCounts,
                    totalForPreviousMonth: totalForPreviousMonthResponse.data.totalForPreviousMonth,
                }));

                const totalBalanceResponse = await axios.get('https://172.188.99.139:5001/api/purchase/purchases-total');
                setCounts(prevCounts => ({
                    ...prevCounts,
