...
const ACForm = ({ onSubmit }) => {
    ...
    useEffect(() => {
        const fetchACPercentage = async () => {
            try {
                const response = await axios.get(`http://172.188.99.139:5000/api/section/${selectedSection}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedSection && acPercentage !== '') {
            try {
                await axios.patch(`http://172.188.99.139:5000/api/section/ac/${selectedSection}`, {
