...
const ACForm = ({ onSubmit }) => {
    ...
    useEffect(() => {
        const fetchACPercentage = async () => {
            try {
                const response = await axios.get(`https://172.188.99.139:5001/api/section/${selectedSection}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedSection && acPercentage !== '') {
            try {
                await axios.patch(`https://172.188.99.139:5001/api/section/ac/${selectedSection}`, {
